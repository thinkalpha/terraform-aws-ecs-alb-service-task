locals {
  ...
  enable_ecs_service_role = module.this.enabled && var.enable_ecs_service_role == true && var.network_mode != "awsvpc" && length(var.ecs_load_balancers) <= 1
...
resource "aws_ecs_service" "ignore_changes_task_definition_and_desired_count" {
  ...
  dynamic "network_configuration" {
    ...
    content {
      security_groups  = compact(concat(module.security_group.*.id, var.security_groups))
      # upstream bug ??? # security_groups  = compact(concat(var.security_group_ids, aws_security_group.ecs_service.*.id))
      subnets          = var.subnet_ids
      assign_public_ip = var.assign_public_ip
    }
  }
